...
public class SlopesPlugin extends JavaPlugin {
	...
	/**
	 * Internal class that overrides get to lazily create the list of blocks for this material if it doesn't exist.
	 */
	...
	public void onLoad() {
		super.onLoad();
		texture = new Texture(this, getResource(TEXTURE_IMAGES), TEXTURE_IMAGES, 256, 64, 16);
	}
	...
	private void generateBlocksForMaterials(final EnumSet<SlopeSubTexture> materialSet, final Class<?>... blockClasses) {
		for (final SlopeSubTexture material : materialSet) {
			...
			for (final Class<?> block : blockClasses) {
				materialBlocks.add(createBlockForSubTexture((Class<? extends AbstractBlock>) block, material));
			}
		}
	}
	 ...
	 * @param blockClass Class to instantiate
